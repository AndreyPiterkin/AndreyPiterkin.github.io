...
(define (suffix-directory? suffix)
  (define-values (base name dir?) (split-path (simplify-path (current-directory)))) ; Extract the last component
  (and (path? name) (string=? (path->string name) suffix))) ; Compare with suffix

(define (find-src-directory [current-dir (current-directory)])
  (if (suffix-directory? "src")
      (build-path "colors.rktd")
      (build-path 'up "colors.rktd")))


(define golden-ratio-conj 0.618033988749895)
(define sat 0.7)
(define val 0.8)
(define colors-file (find-src-directory))
(define prev-hue 0.7)
